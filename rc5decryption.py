...
class RC5D(object):
    ...
    def _expand_key(key, wordsize, rounds):
        ...
        def _mix(L, S, r, w, c):
            ...
            for k in range(3 * m):
                A = S[i] = RC5D._rotate_left(S[i] + A + B, 3, w)
                B = L[j] = RC5D._rotate_left(L[j] + A + B, A + B, w)
    ...
    def _encrypt_block(data, expanded_key, blocksize, rounds):
        ...
        for i in range(1, rounds + 1):
            A = (RC5D._rotate_left((A ^ B), B, w) + expanded_key[2 * i]) % mod
            B = (RC5D._rotate_left((A ^ B), A, w) + expanded_key[2 * i + 1]) % mod
    ...
    def _decrypt_block(data, expanded_key, blocksize, rounds):
        ...
        for i in range(rounds, 0, -1):
            B = RC5D._rotate_right(B - expanded_key[2 * i + 1], A, w) ^ A
            A = RC5D._rotate_right((A - expanded_key[2 * i]), B, w) ^ B
    ...
    def encrypt_file(self, infile, outfile):
        ...
        expanded_key = RC5D._expand_key(self._key, w, self.rounds)
        ...
        while chunk:
            ...
            encrypted_chunk = RC5D._encrypt_block(chunk, expanded_key,
                                                 self.blocksize,
                                                 self.rounds)
    ...
    def decrypt_file(self, infile, outfile):
        ...
        expanded_key = RC5D._expand_key(self._key, w,
                                       self.rounds)

        chunk = infile.read(b)

        while chunk:
            decrypted_chunk = RC5D._decrypt_block(chunk, expanded_key,
                                                 self.blocksize,
                                                 self.rounds)
            if self.mode == 'CBC':
...
#text = input('Insert A Text To Be Encrypted')
#key = input('Enter a key for decrypted this message later ^^) :')
#encryptedStr = rc5.encrypt_str(text)
#print('encryptedStr :', encryptedStr)
# text = input()
# key = input()
# rc5 = RC5D(key)
# print('decryptedStr :', rc5.decrypt_str(text))
